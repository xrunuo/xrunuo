...
namespace Server
{
	...
	public class Season
	{
		...
		{
			var filename = Path.Combine( "Data/Seasons", String.Format( "{0}.cfg", name ) );

			if ( File.Exists( filename ) )
			{
				Dictionary<int, int> tileChanges = new Dictionary<int, int>();

				using ( var bin = new StreamReader( filename ) )
				{
					string line;
					while ( ( line = bin.ReadLine() ) != null )
					{
						...
						{
							log.Warning( "Invalid season entry: {0}", line );
						}
					}
				}
			...
			{
				log.Warning( "Season {0} not found.", name );
				return null;
			}
		}
	}
