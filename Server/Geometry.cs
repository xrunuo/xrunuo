...
namespace Server
{
	...
	public struct Rectangle2D
	{
		...
		{
			Regex pattern = new Regex( @"^\(([0-9]+),([0-9]+)\)\+\(([0-9]+),([0-9]+)\)$", RegexOptions.IgnoreCase );

			try
			{
				Match match = pattern.Match( value.Replace( " ", "" ) );

				int x = Convert.ToInt32( match.Groups[1].Value );
				int y = Convert.ToInt32( match.Groups[2].Value );
				int width = Convert.ToInt32( match.Groups[3].Value );
				int height = Convert.ToInt32( match.Groups[4].Value );
