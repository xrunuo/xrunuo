...
namespace Server.Scripts.Commands
{
	public class Decorate
	{
		...
		{
			CommandSystem.Register( "Decorate", AccessLevel.Developer, new CommandEventHandler( Decorate_OnCommand ) );
			CommandSystem.Register( "SADecorate", AccessLevel.Developer, new CommandEventHandler( SADecorate_OnCommand ) );
		}
		...
		private static void Decorate_OnCommand( CommandEventArgs e )
		{
			...
			{
				Generate( "Data/Decoration/TC/Felucca", Map.Felucca );
				Generate( "Data/Decoration/TC/Trammel", Map.Trammel );
				Generate( "Data/Decoration/TC/Tokuno", Map.Tokuno );
			}
		...
		[Usage( "SADecorate" )]
		[Description( "Generates world decoration." )]
		private static void SADecorate_OnCommand( CommandEventArgs e )
		{
			m_Mobile = e.Mobile;
			m_Count = 0;
			...
			Generate( "Data/Decoration/TerMur", Map.TerMur );
			Generate( "Data/Decoration/Felucca/Stygian Abyss", Map.Felucca );
	...
	public class DecorationList
	{
		...
		private static Type typeofVinesLockedSecretDoor = typeof( VinesLockedSecretDoor );
		private static Type typeofTombOfKingsSecretDoor = typeof( TombOfKingsSecretDoor );
		...
		private static bool FindItem( int x, int y, int z, Map map, Item srcItem )
		{
			...
			{
				foreach ( Item item in map.GetItemsInRange( new Point3D( x, y, z ), 1 ) )
				{
					if ( !( item is BaseDoor ) )
					{
						continue;
