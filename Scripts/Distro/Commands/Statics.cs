...
namespace Server
{
	public class Statics
	{
		...
		{
			CommandSystem.Register( "Freeze", AccessLevel.Developer, new CommandEventHandler( Freeze_OnCommand ) );
			CommandSystem.Register( "FreezeMap", AccessLevel.Developer, new CommandEventHandler( FreezeMap_OnCommand ) );
			CommandSystem.Register( "FreezeWorld", AccessLevel.Developer, new CommandEventHandler( FreezeWorld_OnCommand ) );

			CommandSystem.Register( "Unfreeze", AccessLevel.Developer, new CommandEventHandler( Unfreeze_OnCommand ) );
			CommandSystem.Register( "UnfreezeMap", AccessLevel.Developer, new CommandEventHandler( UnfreezeMap_OnCommand ) );
			CommandSystem.Register( "UnfreezeWorld", AccessLevel.Developer, new CommandEventHandler( UnfreezeWorld_OnCommand ) );
		}
		...
		private const string BaseFreezeWarning = "{0}  " + "Those items <u>will be removed from the world</u> and placed into the server data files.  " + "Other players <u>will not see the changes</u> unless you distribute your data files to them.<br><br>" + "This operation may not complete unless the server and client are using different data files.  " + "If you receive a message stating 'output data files could not be opened,' then you are probably sharing data files.  " + "Create a new directory for the world data files (statics*.mul and staidx*.mul) and add that to Scritps/Misc/DataPath.cs.<br><br>" + "The change will be in effect immediately on the server, however, you must restart your client and update it's data files for the changes to become visible.  " + "It is strongly recommended that you make backup of the data files mentioned above.  " + "Do you wish to proceed?";
		...
		public static void Freeze( Mobile from, Map targetMap, Point3D start3d, Point3D end3d )
		{
			...
			else if ( targetMap != null )
			{
				...
				var eable = targetMap.GetItemsInBounds( new Rectangle2D( start.X, start.Y, end.X - start.X + 1, end.Y - start.Y + 1 ) );
					...
					using ( FileStream mulStream = OpenWrite( matrix.DataStream ) )
					{
						...
						foreach ( DeltaState state in table.Values )
						{
							...
							for ( int i = 0; i < state.m_List.Count; ++i )
							{
								...
								newTile.m_ID = (ushort) ( item.ItemID & TileData.MaxItemValue );
		...
		private const string BaseUnfreezeWarning = "{0}  " + "Those items <u>will be removed from the static files</u> and exchanged with unmovable dynamic items.  " + "Other players <u>will not see the changes</u> unless you distribute your data files to them.<br><br>" + "This operation may not complete unless the server and client are using different data files.  " + "If you receive a message stating 'output data files could not be opened,' then you are probably sharing data files.  " + "Create a new directory for the world data files (statics*.mul and staidx*.mul) and add that to Scritps/Misc/DataPath.cs.<br><br>" + "The change will be in effect immediately on the server, however, you must restart your client and update it's data files for the changes to become visible.  " + "It is strongly recommended that you make backup of the data files mentioned above.  " + "Do you wish to proceed?";
						...
						for ( int y = yStartBlock; y <= yEndBlock; ++y )
						{
							...
							for ( int i = 0; i < oldTileCount; ++i )
							{
								...
								else
								{
									...
									Item item = new Static( oldTile.m_ID & TileData.MaxItemValue );
