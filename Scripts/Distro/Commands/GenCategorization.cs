...
namespace Server.Custom
{
	public class Categorization
	{
		...
		{
			CommandSystem.Register( "RebuildCategorization", AccessLevel.Developer, new CommandEventHandler( RebuildCategorization_OnCommand ) );
		}
		...
		public static void RecurseExport( XmlTextWriter xml, CategoryEntry ce )
		{
			...
			for ( int i = 0; i < ce.Matched.Count; ++i )
			{
				...
				if ( obj is Item )
				{
					...
					if ( item is BaseAddon && ( (BaseAddon) item ).AddonComponents.Count == 1 )
					{
						itemID = ( (AddonComponent) ( ( (BaseAddon) item ).AddonComponents[0] ) ).ItemID;
					}

					if ( item is BaseMulti || itemID > TileData.MaxItemValue )
					{
						itemID = 1;
					}
