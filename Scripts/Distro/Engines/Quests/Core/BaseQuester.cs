...
namespace Server.Engines.Quests
{
	...
	public abstract class BaseQuester : BaseVendor
	{
		...
		public virtual bool NoContextMenu( PlayerMobile to )
		{
			return false;
		}
		...
		public override void AddCustomContextEntries( Mobile from, List<ContextMenuEntry> list )
		{
			...
			if ( from.Alive && from is PlayerMobile && TalkNumber > 0 && CanTalkTo( (PlayerMobile) from ) && !NoContextMenu( (PlayerMobile) from ) )
		...
		public override void OnMovement( Mobile m, Point3D oldLocation )
		{
			if ( m.Alive && m is PlayerMobile )
			{
				...
				if ( m.Alive && range >= 0 && this.InRange( m, range ) && !this.InRange( oldLocation, range ) && CanTalkTo( pm ) )
				{
					OnTalk( pm, false );
				}
			}
