...
namespace Server.Mobiles
{
	public class MageAI : BaseAI
	{
		...
		{
			if ( !m_Mobile.DisallowAllMoves && ( ( m_Mobile.ControlOrder == OrderType.Come || Utility.Random( 4 ) == 0 ) ) )
			{
		...
		public virtual Spell GetRandomManaDrainSpell( Mobile c )
		{
		...
		{
			if ( m_Mobile.Deleted || m_Mobile.Int < 95 )
				return null;

			if ( m_Mobile.ControlOrder == OrderType.Come && m_Mobile.ControlMaster != null )
				return m_Mobile.ControlMaster;

			if ( CanDispel( m_Mobile ) || m_Mobile.AutoDispel )
				return null;
		...
		public virtual bool ProcessTarget()
		{
			...
			if ( isDispel || ( m_Mobile.ControlOrder == OrderType.Come && isTeleport ) )
			{
