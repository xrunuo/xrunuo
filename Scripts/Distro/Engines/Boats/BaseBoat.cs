...
namespace Server.Multis
{
	...
	public abstract class BaseBoat : BaseMulti, IBoat
	{
		...
		private int m_Speed;
		private BoatMovementSpeed m_ClientSpeed;
		...
		public uint CreateKeys( Mobile m )
		{
			...
			else
			{
				...
				m.LocalOverheadMessage( MessageType.Regular, 0x3B2, 502484 );
			}

			if ( m.AddToBackpack( packKey ) )
			{
				// A ship's key is now in my backpack.
				m.LocalOverheadMessage( MessageType.Regular, 0x3B2, 502485 );
			}
			else
			{
				// A ship's key is now at my feet.
				m.LocalOverheadMessage( MessageType.Regular, 0x3B2, 502483 );
			}
		...
		{
			bool forceResend = !this.InRange( old, 1 );

			if ( m_TillerMan != null )
				m_TillerMan.SetLocation( new Point3D( X + ( m_TillerMan.X - old.X ), Y + ( m_TillerMan.Y - old.Y ), Z + ( m_TillerMan.Z - old.Z ) ), forceResend );

			if ( m_Hold != null )
				m_Hold.SetLocation( new Point3D( X + ( m_Hold.X - old.X ), Y + ( m_Hold.Y - old.Y ), Z + ( m_Hold.Z - old.Z ) ), forceResend );

			if ( m_PPlank != null )
				m_PPlank.SetLocation( new Point3D( X + ( m_PPlank.X - old.X ), Y + ( m_PPlank.Y - old.Y ), Z + ( m_PPlank.Z - old.Z ) ), forceResend );

			if ( m_SPlank != null )
				m_SPlank.SetLocation( new Point3D( X + ( m_SPlank.X - old.X ), Y + ( m_SPlank.Y - old.Y ), Z + ( m_SPlank.Z - old.Z ) ), forceResend );
		}
		...
		public bool StartMove( Direction dir, bool fast )
		{
			...
			BoatMovementSpeed clientSpeed = fast ? BoatMovementSpeed.FastMovement : BoatMovementSpeed.VerbalSlowMovement;
		...
		public bool OneMove( Direction dir )
		{
			...
			if ( StartMove( dir, speed, BoatMovementSpeed.OneTileMovement, interval, true, true ) )
			{
				if ( m_TillerMan != null )
					m_TillerMan.Say( 501429 ); // Aye aye sir.
		...
		public bool DoMovement( bool message )
		{
			...
			else
			{
				...
				clientSpeed = BoatMovementSpeed.FastMovement;
			}
