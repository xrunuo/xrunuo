...
namespace Server.Engines.Quests
{
	public class QuestHelper
	{
		...
		public static bool OnDiscorded( PlayerMobile player, Mobile target )
		{
			for ( int i = player.Quests.Count - 1; i >= 0; i-- )
			{
				BaseQuest quest = player.Quests[i];
				...
				for ( int j = quest.Objectives.Count - 1; j >= 0; j-- )
				{
					...
					{
						DiscordObjective obj = (DiscordObjective) quest.Objectives[j];
		...
		{
			return GetQuest<T>( from ) != null;
		}

		public static T GetQuest<T>( PlayerMobile from ) where T : BaseQuest
		{
			for ( int i = from.Quests.Count - 1; i >= 0; i-- )
			{
				BaseQuest quest = from.Quests[i];
