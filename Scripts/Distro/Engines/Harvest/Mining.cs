...
namespace Server.Engines.Harvest
{
	public class Mining : HarvestSystem
	{
		...
		private Mining()
		{
			...
			// Every bank holds from 11 to 24 uses
			oreAndStone.MinTotal = 11;
			oreAndStone.MaxTotal = 24;

			// A resource bank will respawn its content every 6 to 7 minutes
			oreAndStone.MinRespawn = TimeSpan.FromMinutes( 6.0 );
			oreAndStone.MaxRespawn = TimeSpan.FromMinutes( 7.0 );
			...
			veins = new HarvestVein[]
				{
					new HarvestVein( 49.6, res[0] ), // Iron
					new HarvestVein( 11.2, res[1] ), // Dull Copper
					new HarvestVein( 09.8, res[2] ), // Shadow Iron
					new HarvestVein( 08.4, res[3] ), // Copper
					new HarvestVein( 07.0, res[4] ), // Bronze
					new HarvestVein( 05.6, res[5] ), // Gold
					new HarvestVein( 04.2, res[6] ), // Agapite
					new HarvestVein( 02.8, res[7] ), // Verite
					new HarvestVein( 01.4, res[8] )  // Valorite
				};
			...
			res = new HarvestResource[]
				{
					new HarvestResource( 100.0, 0.0, 100.0, 1044631, typeof( Sand ) )
				};

			veins = new HarvestVein[]
				{
					new HarvestVein( 100.0, res[0] )
				};
		...
		public override Type GetResourceType( Mobile from, Item tool, HarvestDefinition def, Map map, Point3D loc, HarvestResource resource )
		{
			if ( def == m_OreAndStone )
			{
				...
				if ( 0.1 > Utility.RandomDouble() )
				{
					...
					if ( pm != null )
						stoneMining = pm.StoneMining && pm.ToggleMiningStone && from.Skills[SkillName.Mining].Base >= 100.0;
				}
		...
		public override bool CheckHarvest( Mobile from, Item tool )
		{
			...
			{
				from.SendLocalizedMessage( 501864 ); // You can't mine while riding or flying.
				return false;
			}
		...
		public override void OnHarvestFinished( Mobile from, Item tool, HarvestDefinition def, HarvestVein vein, HarvestBank bank, HarvestResource resource, object harvested )
		{
			...
			{
				HarvestResource res = vein.Resource;
