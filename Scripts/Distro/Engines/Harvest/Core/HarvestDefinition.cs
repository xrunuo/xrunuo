...
namespace Server.Engines.Harvest
{
	public class HarvestDefinition
	{
		...
		public HarvestBank GetBank( Map map, int x, int y )
		{
			...
			if ( bank == null )
			{
				HarvestVein vein = GetVeinAt( map, x, y );
				double chanceToFallback = GetChanceToFallbackAt( map, x, y );
				banks[key] = bank = new HarvestBank( this, vein, chanceToFallback );
			}

			return bank;
		}

		private HarvestVein GetVeinAt( Map map, int x, int y )
		{
			if ( m_Veins.Length == 1 )
				return m_Veins[0];
			...
			if ( m_RandomizeVeins )
				...
				random = GetPseudoRandom( map, x, y );

			return GetVeinFrom( random.NextDouble() );
		}

		private double GetChanceToFallbackAt( Map map, int x, int y )
		{
			Random random = GetPseudoRandom( map, x, y );

			double u1 = random.NextDouble();
			double u2 = random.NextDouble();

			double rand = Math.Sqrt( -2.0 * Math.Log( u1 ) ) * Math.Sin( 2.0 * Math.PI * u2 );
		...
		{
			return Utility.RandomGenerator;
		}

		private Random GetPseudoRandom( Map map, int x, int y )
		{
			return new Random( ( x * 17 ) + ( y * 11 ) + ( map.MapID * 3 ) );
		}
		...
		public HarvestVein GetVeinFrom( double randomValue )
		{
			...
			{
				if ( randomValue <= m_Veins[i].Chance )
					return m_Veins[i];

				randomValue -= m_Veins[i].Chance;
			}
