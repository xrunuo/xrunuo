...
namespace Server.Engines.Harvest
{
	public abstract class HarvestSystem
	{
		...
		public virtual void FinishHarvesting( Mobile from, Item tool, HarvestDefinition def, object toHarvest, object locked )
		{
			...
			HarvestResource primary = vein.Resource;
			HarvestResource fallback = def.Resources[0];
			HarvestResource resource = MutateResource( from, tool, def, map, loc, bank, primary, fallback );
				...
				if ( type != null )
				{
					...
					else
					{
						if ( item.Stackable )
						{
							...
							int racialAmount = (int) ( amount * 1.1 );
							int feluccaRacialAmount = (int) ( feluccaAmount * 1.1 );

							bool eligableForRacialBonus = ( def.RaceBonus && from.Race == Race.Human );
							bool inFelucca = ( map == Map.Felucca );

							if ( eligableForRacialBonus && inFelucca )
								item.Amount = feluccaRacialAmount;
							else if ( inFelucca )
								item.Amount = feluccaAmount;
							else if ( eligableForRacialBonus )
		...
		public virtual HarvestResource MutateResource( Mobile from, Item tool, HarvestDefinition def, Map map, Point3D loc, HarvestBank bank, HarvestResource primary, HarvestResource fallback )
		{
			bool racialBonus = ( def.RaceBonus && from.Race == Race.Elf );

			if ( bank.ChanceToFallback > ( Utility.RandomDouble() + ( racialBonus ? .20 : 0 ) ) )
				return fallback;
		...
		public virtual bool GetHarvestDetails( Mobile from, Item tool, object toHarvest, out int tileID, out Map map, out Point3D loc )
		{
			if ( toHarvest is Static && !( (Static) toHarvest ).Movable )
			{
				...
				tileID = ( obj.ItemID & 0x7FFF ) | 0x8000;
				map = obj.Map;
