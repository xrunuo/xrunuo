...
namespace Server.Factions
{
	...
	public class SpellCombo
	{
		...
		public static readonly SpellCombo Simple = new SpellCombo( 50, new ComboEntry( typeof( ParalyzeSpell ), 20 ), new ComboEntry( typeof( ExplosionSpell ), 100, TimeSpan.FromSeconds( 2.8 ) ), new ComboEntry( typeof( PoisonSpell ), 30 ), new ComboEntry( typeof( EnergyBoltSpell ) ) );

		public static readonly SpellCombo Strong = new SpellCombo( 90, new ComboEntry( typeof( ParalyzeSpell ), 20 ), new ComboEntry( typeof( ExplosionSpell ), 50, TimeSpan.FromSeconds( 2.8 ) ), new ComboEntry( typeof( PoisonSpell ), 30 ), new ComboEntry( typeof( ExplosionSpell ), 100, TimeSpan.FromSeconds( 2.8 ) ), new ComboEntry( typeof( EnergyBoltSpell ) ), new ComboEntry( typeof( PoisonSpell ), 30 ), new ComboEntry( typeof( EnergyBoltSpell ) ) );
	...
	public class FactionGuardAI : BaseAI
	{
		...
		public bool StartBandage()
		{
			...
			Bandage bandage = pack.FindItemByType( typeof( Bandage ) ) as Bandage;

			if ( bandage == null )
				return false;

			m_Bandage = BandageContext.BeginHeal( m_Guard, m_Guard, bandage );
