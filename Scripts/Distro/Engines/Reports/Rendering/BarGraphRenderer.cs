...
namespace Server.Engines.Reports
{
	...
	public class BarGraphRenderer : ChartRenderer
	{
		...
		public BarGraphRenderMode RenderMode { get { return _renderMode; } set { _renderMode = value; } }

		public Color BackgroundColor { set { _backColor = value; } }

		public int BottomBuffer { set { _bottomBuffer = Convert.ToSingle( value ); } }

		public Color FontColor { set { _fontColor = value; } }

		public int Height { get { return Convert.ToInt32( _totalHeight ); } set { _totalHeight = Convert.ToSingle( value ); } }

		public int Width { get { return Convert.ToInt32( _totalWidth ); } set { _totalWidth = Convert.ToSingle( value ); } }

		public bool ShowLegend { get { return _displayLegend; } set { _displayLegend = value; } }

		public bool ShowData { get { return _displayBarData; } set { _displayBarData = value; } }
		public int TopBuffer { set { _topBuffer = Convert.ToSingle( value ); } }

		public string VerticalLabel { get { return _yLabel; } set { _yLabel = value; } }

		public int VerticalTickCount { get { return _yTickCount; } set { _yTickCount = value; } }
					...
					using ( SolidBrush barBrush = new SolidBrush( item.ItemColor ) )
					{
						using ( Pen pen = new Pen( _fontColor, 0.15f ) )
						{
							...
							else if ( _renderMode == BarGraphRenderMode.Bars )
							{
								...
								PointF[] pts = new PointF[] { new PointF( ox, oy ), new PointF( ox + ow, oy ), new PointF( ox + of, oy + of ), new PointF( ox + of + ow, oy + of ), new PointF( ox, oy + oh ), new PointF( ox + of, oy + of + oh ), new PointF( ox + of + ow, oy + of + oh ) };
								...
								if ( _displayBarData && ( i % _interval ) == 0 )
								{
									...
									using ( SolidBrush brsh = new SolidBrush( Color.FromArgb( 128, Color.White ) ) )
									{
										for ( int box = -1; box <= 1; ++box )
										{
											for ( int boy = -1; boy <= 1; ++boy )
											{
												...
												graph.DrawString( item.Value.ToString( "#,###.##" ), valFont, brsh, rco, sfFormat );
											}
										}
									}
