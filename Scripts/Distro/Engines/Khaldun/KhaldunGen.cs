...
namespace Server.Scripts.Commands
{
	public class GenKhaldun
	{
		...
		{
			CommandSystem.Register( "GenKhaldun", AccessLevel.Developer, new CommandEventHandler( GenKhaldun_OnCommand ) );
		}

		public static bool FindMorphItem( int x, int y, int z, int inactiveItemID, int activeItemID )
		{
			return Map.Felucca.GetItemsInRange( new Point3D( x, y, z ), 0 )
				.OfType<MorphItem>()
				.Where( item => item.Z == z && item.InactiveItemID == inactiveItemID && item.ActiveItemID == activeItemID )
				.Any();
		}
		...
		{
			var item = Map.Felucca.GetItemsInBounds( new Rectangle2D( x, y, 1, 1 ) )
				.Where( i => i.GetType() == srcItem.GetType() )
				.FirstOrDefault();
		...
		[Usage( "GenKhaldun" )]
		[Description( "Genera objetos y teleports dinamicos de la dungeon Khaldun." )]
