...
namespace Server.Items
{
	public class RaiseSwitch : Item
	{
		...
		public RaisableItem RaisableItem { get { return m_RaisableItem; } set { m_RaisableItem = value; } }
	...
	public class DisappearingRaiseSwitch : RaiseSwitch
	{
		...
		{
			if ( this.InRange( m, CurrentRange ) || this.InRange( oldLocation, CurrentRange ) )
				Refresh();
		}
		...
		public void Refresh()
		{
			Visible = GetMobilesInRange( CurrentRange ).Cast<Mobile>()
				.Any( m => !m.Hidden || m.AccessLevel == AccessLevel.Player );
		}
