...
namespace Server.Engines.Craft
{
	...
	{
		bool OnCraft( bool exceptional, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, BaseTool tool, CraftItem craftItem, int resHue );
	}
	...
	public class CraftItem : IComparable<CraftItem>
	{
		...
		private bool m_RequiresML;
		private bool m_RequiresSA;
		private bool m_RequiresHS;

		private bool m_ForceCannotEnhance;

		public bool ForceCannotEnhance
		{
			get { return m_ForceCannotEnhance; }
			set { m_ForceCannotEnhance = value; }
		}
		...
		private BeverageType m_BeverageType = BeverageType.Water;

		public BeverageType BeverageType
		{
			get { return m_BeverageType; }
			set { m_BeverageType = value; }
		}

		private bool m_OverridesChanceAtMin;
		private double m_ChanceAtMin;

		public bool OverridesChanceAtMin
		{
			get { return m_OverridesChanceAtMin; }
			set { m_OverridesChanceAtMin = value; }
		}

		public double ChanceAtMin
		{
			get { return m_ChanceAtMin; }
			set
			{
				m_ChanceAtMin = value;
				m_OverridesChanceAtMin = true;
			}
		}
		...
		public static int ItemIDOf( Type type )
		{
			...
			if ( !m_ItemIds.TryGetValue( type, out itemId ) )
			{
		...
		public bool RequiresML { get { return m_RequiresML; } set { m_RequiresML = value; } }
		public bool RequiresSA { get { return m_RequiresSA; } set { m_RequiresSA = value; } }
		public bool RequiresHS { get { return m_RequiresHS; } set { m_RequiresHS = value; } }
		...
		public static Type[][] TypesTable { get { return m_TypesTable; } }
		...
		private static Type[][] m_TypesTable = new Type[][] 
		{
			...
			new Type[] {typeof( BowlOfPeas ), typeof( PewterBowl )}
		};
		...
		private static Type[] m_ColoredResourceTable = new Type[] 
		{
			...
			typeof( UncutCloth ), typeof( Cloth ), typeof( BoltOfCloth ), 
			typeof( BaseGranite ), 
			typeof( BaseScales )
		};
		...
		private static Type[] m_MarkableTable = new Type[]
		{
			...
			typeof( BaseClothing ), typeof( BaseInstrument ),
			typeof( DragonBardingDeed ), typeof( BaseTool ),
			typeof( FukiyaDart ), typeof( Shuriken ),
			typeof( Fukiya ), typeof( LeatherNinjaBelt ),
			typeof( Spellbook ), typeof( Runebook ),
			typeof( KeyRing ), typeof( IronKey ),
			typeof( BaseJewel ), typeof( Clippers ),
			typeof( Food )
		};
		...
		public int ConsumeQuantity( Container cont, Type[][] types, int[] amounts )
		{
			...
			for ( int i = 0; i < types.Length; ++i )
			{
				...
				for ( int j = 0; j < items[i].Length; ++j )
				{
					...
					{
						if ( hq is BaseBeverage && ( (BaseBeverage) hq ).Content != m_BeverageType )
							continue;
		...
		public double GetExceptionalChance( CraftSystem system, double chance, Mobile from )
		{
			...
			double bonustalis = 0.0;

			if ( chance > 0.0 )
			{
				BaseTalisman talis = BaseTalisman.GetTalisman( from );

				if ( talis != null && Enum.GetName( typeof( CraftList ), talis.CraftBonusExcep ) == system.MainSkill.ToString() && talis.CraftBonusExcepValue > 0 )
					bonustalis = (double) talis.CraftBonusExcepValue / 100;
			}
		...
		public bool CheckSkills( Mobile from, Type typeRes, CraftSystem craftSystem, ref bool exceptional, ref bool allRequiredSkills )
		{
			return CheckSkills( from, typeRes, craftSystem, ref exceptional, ref allRequiredSkills, true );
		}

		public bool CheckSkills( Mobile from, Type typeRes, CraftSystem craftSystem, ref bool exceptional, ref bool allRequiredSkills, bool gainSkills )
		{
			...
			if ( chance > 0.0 )
			{
				// El bonus regular de los talismanes no debe afectar al exceptional.
				chance += GetTalismanBonus( from, craftSystem );
			}

			return ( chance > Utility.RandomDouble() );
		}

		public double GetTalismanBonus( Mobile from, CraftSystem craftSystem )
		{
			BaseTalisman talisman = BaseTalisman.GetTalisman( from );

			if ( talisman != null && talisman.CraftBonusRegularValue > 0 )
			{
				if ( Enum.GetName( typeof( CraftList ), talisman.CraftBonusRegular ) == craftSystem.MainSkill.ToString() )
					return (double) talisman.CraftBonusRegularValue / 100;
			}
		...
		public void Craft( Mobile from, CraftSystem craftSystem, Type typeRes, BaseTool tool )
		{
			if ( from.BeginAction( typeof( CraftSystem ) ) )
			{
				if ( ItemType == null )
				{
					...
					// No such object.
					from.SendGump( new CraftGump( from, craftSystem, tool, 500032 ) );
				...
				{
					if ( from is PlayerMobile && !( (PlayerMobile) from ).CanCraftPets )
					{
						from.EndAction( typeof( CraftSystem ) );
				...
				if ( RequiresHS && !Expansion.HS )
				{
					from.EndAction( typeof( CraftSystem ) );
					from.SendGump( new CraftGump( from, craftSystem, tool, 1116295 ) ); // You must have the High Seas booster pack to use this feature
				}
				else
				{
		...
		public void CompleteCraft( bool exceptional, bool makersMark, bool questItem, Mobile from, CraftSystem craftSystem, Type typeRes, BaseTool tool, CustomCraft customCraft )
		{
			int badCraft = craftSystem.CanCraft( from, tool, m_Type );
			...
			if ( CheckSkills( from, typeRes, craftSystem, ref ignored, ref allRequiredSkills ) )
			{
				...
				if ( item != null )
				{
					...
					{
						if ( !isArtifact )
							endExceptional = ( (ICraftable) item ).OnCraft( exceptional, makersMark, from, craftSystem, typeRes, tool, this, resHue );
					}
					else if ( item.Hue == 0 )
					{
						item.Hue = resHue;
					...
					{
						if ( questItem && from is PlayerMobile && !QuestHelper.CheckItem( (PlayerMobile) from, item ) )
							from.SendLocalizedMessage( 1072355, null, 0x23 ); // That item does not match any of your quest criteria
					}
				}

				if ( num == 0 )
					num = craftSystem.PlayEndingEffect( from, false, true, toolBroken, endExceptional, makersMark, this );
				...
				if ( queryFactionImbue )
					from.SendGump( new FactionImbueGump( endExceptional, item, from, craftSystem, tool, num, availableSilver, faction, def ) );
			...
			else
			{
				...
				// SkillCheck failed.
				int num = craftSystem.PlayEndingEffect( from, true, true, toolBroken, endExceptional, false, this );
		...
		private class InternalTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				else
				{
					...
					m_CraftItem.CheckSkills( m_From, m_TypeRes, m_CraftSystem, ref exceptional, ref allRequiredSkills, false );
					...
					if ( typeof( CustomCraft ).IsAssignableFrom( m_CraftItem.ItemType ) )
					{
						...
						try { cc = Activator.CreateInstance( m_CraftItem.ItemType, new object[] { m_From, m_CraftItem, m_CraftSystem, m_TypeRes, m_Tool, exceptional } ) as CustomCraft; }
						catch { }
					...
					bool questItem = context.QuestItem;

					if ( exceptional && m_From.Skills[m_CraftSystem.MainSkill].Base >= 100.0 )
						makersMark = m_CraftItem.IsMarkable( m_CraftItem.ItemType );

					if ( makersMark && context.MarkOption == CraftMarkOption.PromptForMark )
					{
						m_From.SendGump( new QueryMakersMarkGump( exceptional, m_From, m_CraftItem, m_CraftSystem, m_TypeRes, m_Tool, questItem ) );
					}
					else
					{
						...
						m_CraftItem.CompleteCraft( exceptional, makersMark, context.QuestItem, m_From, m_CraftSystem, m_TypeRes, m_Tool, null );
					}
				}
			}
		}

		public int CompareTo( CraftItem other )
		{
			if ( other == null )
				return -1;

			return m_CraftId.CompareTo( other.CraftId );
		}
	}
}
