...
namespace Server.Engines.Housing.Multis
{
	public abstract class BaseHouse : BaseMulti, IHouse
	{
		...
		public virtual TimeSpan DecayPeriod { get { return TimeSpan.FromDays( 7.0 ); } }
		...
		private static IHouse GetNewsestHouse( Account acct )
		{
		...
		private DecayLevel m_DecayLevel;

		[CommandProperty( AccessLevel.GameMaster )]
		public DecayLevel DecayLevel
		{
			get { return m_DecayLevel; }
		}

		private DecayLevel GetOldDecayLevel()
		{
			...
			foreach ( Item addon in Addons )
			{
				if ( !addon.Deleted )
				{
					...
					if ( addon is BaseAddon )
						deed = ( (BaseAddon) addon ).Redeed();
					else if ( addon is IAddon )
					...
					if ( !item.Deleted )
					{
						if ( item is IAddon )
						{
							if ( ( (IAddon) item ).CouldFit( location, this.Map ) )
							{
								...
								{
									ParrotPerchAddon perch = item as ParrotPerchAddon;

									if ( perch.Parrot != null && !perch.Parrot.Deleted )
										perch.Parrot.MoveToWorld( new Point3D( perch.Location.X, perch.Location.Y, perch.Location.Z + 11 ), perch.Map );
								}
						...
						else
						{
							...
							{
								relocateItem = ( (BaseAddon) item ).Redeed();
								item.Delete();
							}
		...
		public ArrayList GetItems()
		{
			...
			var eable = this.Map.GetItemsInBounds( rect );
		...
		public ArrayList GetMobiles()
		{
			...
			{
				if ( IsInside( mobile ) && !( mobile is Parrot ) )
				{
					list.Add( mobile );
				}
			}
		...
		private class TransferItem : Item
		{
			...
			public override void GetProperties( ObjectPropertyList list )
			{
				...
				HousePlacementEntry hpe = this.m_House.GetAosEntry();

				int houseType = 0;

				if ( hpe != null )
					houseType = hpe.Description;

				int price = this.m_House.Price;
				...
				if ( houseType != 0 )
					list.Add( 1074784, price.ToString() ); // Value: ~1_val~
			}
		...
		public void BeginConfirmTransfer( Mobile from, Mobile to )
		{
			...
			else if ( to.IsPlayer )
			{
				...
				else if ( BaseHouse.HasAccountHouse( to ) )
				{
					from.SendLocalizedMessage( 501388 ); // You cannot transfer ownership to another house owner or co-owner!
				}
		...
		public void AddSecure( Mobile m, Item item )
		{
			...
			else
			{
				...
				if ( info != null )
				{
					m.SendGump( new SetSecureLevelGump( m_Owner, info ) );
				}
		...
		public virtual bool IsCombatRestricted( Mobile m )
		{
			...
			for ( int i = 0; i < m.Aggressed.Count; ++i )
			{
				...
				if ( info.Defender.IsPlayer && info.Defender.Alive && ( DateTime.Now - info.LastCombatTime ) < HouseRegion.CombatHeatDelay && ( attackerGuild == null || defenderGuild == null || defenderGuild != attackerGuild && !defenderGuild.IsEnemy( attackerGuild ) ) )
				{
					return true;
				}
			}
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 15:
					{
						m_CurrentStage = (DecayLevel) reader.ReadInt();
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public int Visits { get { return m_Visits; } set { m_Visits = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public int MaxSecures { get { return m_MaxSecures; } set { m_MaxSecures = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public int MaxLockDowns { get { return m_MaxLockDowns; } set { m_MaxLockDowns = value; } }
		...
		public DateTime BuiltOn { get { return m_BuiltOn; } set { m_BuiltOn = value; } }

		public DateTime LastTraded { get { return m_LastTraded; } set { m_LastTraded = value; } }
		...
		new public bool IsLockedDown( Item check )
		{
			if ( check == null )
				return false;
		...
		new public bool IsSecure( Item item )
		{
			if ( item == null )
				return false;
		...
		public virtual Guildstone FindGuildstone()
		{
			...
			MultiComponentList mcl = Components;
			var eable = map.GetItemsInBounds( new Rectangle2D( X + mcl.Min.X, Y + mcl.Min.Y, mcl.Width, mcl.Height ) );
	...
	public class RelocatedEntity
	{
		...
		public Point3D RelativeLocation { get { return m_RelativeLocation; } }
	...
	public class LockdownTarget : Target
	{
		...
		protected override void OnTarget( Mobile from, object targeted )
		{
			...
			if ( targeted is Item )
			{
				...
				{
					if ( targeted is DecorativeCarpet )
					{
						/* Decorative carpet cannot be locked down in this manner. Simply place
						 * the tile on the floor, and double click it to lock it in place. Double
						 * clicking again will release it. */
						( (DecorativeCarpet) targeted ).SendLocalizedMessageTo( from, 1113269 );
					}
					else if ( targeted is VendorRentalContract || ( targeted is Container && ( (Container) targeted ).FindItemByType( typeof( VendorRentalContract ) ) != null ) )
					{
