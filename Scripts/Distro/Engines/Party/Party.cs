...
namespace Server.Engines.PartySystem
{
	public class Party : IParty
	{
		...
		[Usage( "ListenToParty" )]
		[Description( "Selecciona un jugador para escuchar todo lo que se hable por el chat de su party." )]
			...
			protected override void OnTick()
			{
				...
				foreach ( PartyMemberInfo mi in p.Members )
				{
					...
					if ( m != m_Mobile )
					{
						...
						m_Mobile.Send( new DisplayWaypoint( m, WaypointType.PartyMember, false, 1062613, m.Name ) );
					}
				}
		...
		public void Disband()
		{
			...
			for ( int i = 0; i < m_Members.Count; ++i )
			{
				...
				{
					Mobile f = this[j].Mobile;

					if ( m != f )
						m.Send( new RemoveWaypoint( f.Serial ) );
				}
			}
		...
		public void SendPrivateMessage( Mobile from, Mobile to, string text )
		{
			...
			foreach ( GameClient ns in from.GetClientsInRange( 8 ) )
			{
				...
				{
					if ( p == null )
						p = Packet.Acquire( new UnicodeMessage( from.Serial, from.Body, MessageType.Regular, from.SpeechHue, 3, from.Language, from.Name, String.Format( "[Party]->[{0}]: {1}", to.Name, text ) ) );
		...
		public PartyMemberInfo this[Mobile m]
		{
			get
			{
				return m_Members.FirstOrDefault( pmi => pmi.Mobile == m );
			}
		}
	}
}