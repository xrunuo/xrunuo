...
namespace Server.Accounting
{
	public class Account : IAccount
	{
		...
		public static string HashMD5( string plainPassword )
		{
		...
		/// <summary>
		/// Constructs a new Account instance with a specific username, password amd email. Intended to be only called from Utility.AddAccount.
		/// </summary>
		/// <param name="username">Initial username for this account.</param>
		/// <param name="password">Initial password for this account.</param>
		/// <param name="email">Initial email for this account.</param>
		public Account( string username, string password, string email = null )
		{
			m_Username = username;
			...
			if ( addressList != null )
			{
				...
				foreach ( XmlElement ip in addressList.GetElementsByTagName( "ip" ) )
				{
					try
					{
						...
						{
							list[count] = IPAddress.Parse( Utility.GetText( ip, null ) );
							count++;
						}
					}
					catch
		...
		public void LogAccess( NetState client )
		{
			...
			IPAddress ipAddress = client.Address;
		...
		public void Save( XmlTextWriter xml )
		{
			...
			xml.WriteString( ( (int) m_Features ).ToString() );
			xml.WriteEndElement();
		...
		private AccountFeature m_Features = AccountFeature.None;
		...
		public int Limit
		{
			get { return FeatureEnabled( AccountFeature.SeventhCharacter ) ? 7 : 6; }
		}
		...
		public IEnumerable<Mobile> GetCharacters()
		{
			return m_Mobiles.Where( m => m != null && !m.Deleted );
		}
