...
namespace Server.Items
{
	public abstract class BaseOre : Item, ICommodity
	{
		...
		public override LocalizedText GetNameProperty()
		{
			if ( Amount > 1 )
			{
				return new LocalizedText( 1050039, "{0}\t#{1}", Amount, 1026583 ); // ~1_NUMBER~ ~2_ITEMNAME~
			}
			else
			{
				return new LocalizedText( 1026583 ); // ore
			}
		}
		...
		private class InternalTarget : Target
		{
			...
			private bool IsForge( object obj )
			{
				...
				if ( obj is BaseCreature )
				{
					...
					if ( bc is FireBeetle && bc.Alive && bc.Controlled && bc.ControlOrder == OrderType.Stay )
						bodyID = ( (Mobile) obj ).Body;
				}
				else if ( obj is Item )
					itemID = ( (Item) obj ).ItemID;
				else if ( obj is StaticTarget )
					itemID = ( (StaticTarget) obj ).ItemID & TileData.MaxItemValue;

				return ( itemID == 4017 || ( itemID >= 6522 && itemID <= 6569 ) || bodyID == 0xA9 );
			}
