	...
	public class Aquarium : BaseAddonContainer
	{
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			if ( m_Timer != null )
				...
				writer.Write( DateTime.UtcNow + TimeSpan.FromHours( 24 ) );
				...
				case 1:
					{
						...
						m_Timer = Timer.DelayCall( next - DateTime.UtcNow, TimeSpan.FromHours( 24 ), new TimerCallback( Evaluate ) );
		...
		public virtual bool AddDecoration( Mobile from, Item item )
		{
			...
			if ( item.LabelNumber != 0 )
				from.SendLocalizedMessage( 1073635, String.Format( "#{0}", item.LabelNumber ) ); // You add the following decoration to your aquarium: ~1_NAME~
			else
