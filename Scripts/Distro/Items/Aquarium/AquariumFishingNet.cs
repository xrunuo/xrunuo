...
namespace Server.Items
{
	public class AquariumFishingNet : Item
	{
		...
		{
			if ( 0.004 * skill > Utility.RandomDouble() )
				return new Shrimp();
			if ( 0.008 * skill > Utility.RandomDouble() )
				return new SmallMouthSuckerFin();
			if ( 0.008 * skill > Utility.RandomDouble() )
				return new SpinedScratcherFish();
			if ( 0.022 * skill > Utility.RandomDouble() )
				return new SpottedBuccaneer();
			if ( 0.022 * skill > Utility.RandomDouble() )
				return new YellowFinBluebelly();
			if ( 0.030 * skill > Utility.RandomDouble() )
				return new BritainCrownFish();
			if ( 0.030 * skill > Utility.RandomDouble() )
				return new PurpleFrog();
			if ( 0.032 * skill > Utility.RandomDouble() )
				return new VesperReefTiger();
			if ( 0.038 * skill > Utility.RandomDouble() )
				return new KillerFrog();
			if ( 0.040 * skill > Utility.RandomDouble() )
				return new AlbinoFrog();
			if ( 0.046 * skill > Utility.RandomDouble() )
				return new LongClawCrab();
			if ( 0.052 * skill > Utility.RandomDouble() )
				return new SpeckledCrab();
			if ( 0.058 * skill > Utility.RandomDouble() )
				return new Jellyfish();
			if ( 0.062 * skill > Utility.RandomDouble() )
				return new NujelmHoneyFish();
			if ( 0.064 * skill > Utility.RandomDouble() )
				return new MakotoCourtesanFish();
			if ( 0.076 * skill > Utility.RandomDouble() )
				return new AlbinoCourtesanFish();
			if ( 0.082 * skill > Utility.RandomDouble() )
				return new RedDartFish();
			if ( 0.088 * skill > Utility.RandomDouble() )
				return new MinocBlueFish();
			if ( 0.090 * skill > Utility.RandomDouble() )
				return new GoldenBroadtail();
			if ( 0.148 * skill > Utility.RandomDouble() )
