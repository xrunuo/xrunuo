	...
	public class VendorRentalContract : Item
	{
		...
		private class RentTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				var house = HousingHelper.FindHouseAt( pLocation, map, 0 );

				if ( house == null || !house.IsOwner( from ) )
				{
					from.SendLocalizedMessage( 1062338 ); // The location being rented out must be inside of your house.
				}
				else if ( HousingHelper.FindHouseAt( from ) != house )
				{
				...
				else
				{
					...
					HousingHelper.IsThereVendor( pLocation, map, out vendor, out contract );
