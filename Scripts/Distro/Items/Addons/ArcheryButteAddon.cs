...
namespace Server.Items
{
	...
	public class ArcheryButte : AddonComponent
	{
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public DateTime LastUse { get { return m_LastUse; } set { m_LastUse = value; } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool FacingEast { get { return ( ItemID == 0x100A ); } set { ItemID = value ? 0x100A : 0x100B; } }

		[CommandProperty( AccessLevel.GameMaster )]
		public int Arrows { get { return m_Arrows; } set { m_Arrows = value; } }

		[CommandProperty( AccessLevel.GameMaster )]
		public int Bolts { get { return m_Bolts; } set { m_Bolts = value; } }
		...
		public void Fire( Mobile from )
		{
			...
			BaseThrowing boomerang = from.Weapon as BaseThrowing;

			if ( bow == null && boomerang == null )
			{
			...
			if ( bow != null )
			{
				...
				isArrow = ( ammoType == typeof( Arrow ) );
				isBolt = ( ammoType == typeof( Bolt ) );
				isKnown = ( isArrow || isBolt );
			...
			{
				Effects.SendPacket( from, from.Map, new GraphicalEffect( EffectType.Moving, from.Serial, this.Serial, boomerang.ItemID, from.Location, this.Location, 18, 0, false, 2 ) );
			}

			ScoreEntry se = GetEntryFor( from );

			if ( !from.CheckSkill( weapon.Skill, m_MinSkill, m_MaxSkill ) )
			{
				from.PlaySound( weapon.MissSound );
	...
	public class ArcheryButteAddon : BaseAddon
	{
		...
		private ArcheryButteType m_ArcheryButteType;

		[Constructable]
		public ArcheryButteAddon( ArcheryButteType type )
		{
			//AddComponent( new ArcheryButte( 0x100A ), 0, 0, 0 );
			m_ArcheryButteType = type;

			switch ( type )
			{
				case ArcheryButteType.South:
					AddComponent( new ArcheryButte( 0x100B ), 0, 0, 0 );
					break;
				case ArcheryButteType.East:
					AddComponent( new ArcheryButte( 0x100A ), 0, 0, 0 );
					break;
	...
	{
		public override BaseAddon Addon { get { return new ArcheryButteAddon( m_ArcheryButteType ); } }
		...
		{
			list.Add( (int) ArcheryButteType.South, 1080204 );
			list.Add( (int) ArcheryButteType.East, 1080203 );
		}


		public void OnOptionSelected( Mobile from, int choice )
		{
			m_ArcheryButteType = (ArcheryButteType) choice;
