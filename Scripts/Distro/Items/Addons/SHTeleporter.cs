...
namespace Server.Items
{
	public class SHTeleComponent : AddonComponent
	{
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public Point3D TeleOffset { get { return m_TeleOffset; } set { m_TeleOffset = value; } }

		[CommandProperty( AccessLevel.Counselor )]
		public Point3D TelePoint { get { return new Point3D( Location.X + TeleOffset.X, Location.Y + TeleOffset.Y, Location.Z + TeleOffset.Z ); } set { m_TeleOffset = new Point3D( value.X - Location.X, value.Y - Location.Y, value.Z - Location.Z ); } }
	...
	public class SHTeleporter : BaseAddon
	{
		...
		{
			CommandSystem.Register( "SHTelGen", AccessLevel.Developer, new CommandEventHandler( SHTelGen_OnCommand ) );
		}
