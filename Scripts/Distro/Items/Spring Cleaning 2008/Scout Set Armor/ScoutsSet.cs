...
namespace Server.Items
{
	public class ScoutsSet
	{
		...
		public static bool FullSet( Mobile m )
		{
			...
			return ( chest is ScoutsFemaleStuddedArmor || chest is ScoutsStuddedTunic || chest is ScoutsStuddedBustier )
				&& ( pants is ScoutsStuddedLeggings )
				&& ( arms is ScoutsStuddedSleeves )
				&& ( gloves is ScoutsStuddedGloves )
				&& ( head is ScoutsCirclet || head is ScoutsJingasa )
				&& ( gorget is ScoutsStuddedGorget );
		}
		...
		public static void ApplyBonus( Mobile m )
		{
			...
			mods.Add( new AttributeMod( AosAttribute.BonusDex, 6 ) );
			mods.Add( new AttributeMod( AosAttribute.RegenHits, 2 ) );
			mods.Add( new AttributeMod( AosAttribute.RegenMana, 2 ) );
			mods.Add( new AttributeMod( AosAttribute.AttackChance, 10 ) );
			mods.Add( new AttributeMod( AosAttribute.WeaponDamage, 10 ) );
			...
			m.AddStatMod( new StatMod( StatType.Dex, "ScoutsSetDex", 6, TimeSpan.Zero ) );
		...
		public static void GetPropertiesFirst( ObjectPropertyList list, Item item )
		{
			...
			if ( parent != null && m_Bonus.ContainsKey( parent ) )
			{
				...
				list.Add( 1151216, "10" ); // damage increase ~1_val~% (total)
		...
		public static void GetPropertiesSecond( ObjectPropertyList list, Item item )
		{
			...
			if ( parent == null || !m_Bonus.ContainsKey( parent ) )
			{
				...
				list.Add( 1080447, "6" ); // dexterity bonus +~1_val~ 
				...
				list.Add( 1060401, "10" ); // damage increase ~1_val~%
