...
namespace Server.Items
{
	public class BallOfSummoning : Item, TranslocationItem
	{
		...
		{
			return new LocalizedText( 1054131, m_Charges.ToString() + ( m_PetName.Length == 0 ? "\t " : "\t" + m_PetName ) ); // a crystal ball of pet summoning: [charges: ~1_charges~] : [linked pet: ~2_petName~]
		}
		...
		public void SummonPet( Mobile from )
		{
			...
			if ( from.Mounted && from.Mount == pet )
