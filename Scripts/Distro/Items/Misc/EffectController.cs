...
namespace Server.Items
{
	...
	public class EffectController : Item
	{
		...
		*/
		...
		{
			if ( m.Location != oldLocation && m_TriggerType == EffectTriggerType.InRange && m.InRange( GetWorldLocation(), m_TriggerRange ) && !GetWorldLocation().InRange( oldLocation, m_TriggerRange ) )
			{
				DoEffect( m );
			}
		}
		...
		{
			return World.Instance.FindEntity( reader.ReadInt() );
		}
		...
		public void PlaySound( object trigger )
		{
			...
			Effects.PlaySound( ( ent is Item ) ? ( (Item) ent ).GetWorldLocation() : ent.Location, ent.Map as Map, m_SoundID );
		}
