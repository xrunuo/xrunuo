		...
		public override void OnCast()
		{
			if ( CheckSequence() )
			{
				...
				foreach ( Mobile m in Caster.GetMobilesInRange( 3 ) ) // TODO: Validate range
				{
					...
					if ( Caster != m && Caster.CanSee( m ) && Caster.InLOS( m ) && Caster.CanBeBeneficial( m, false, true ) && !( m is BaseCreature && ( (BaseCreature) m ).IsGolem ) )
						targets.Add( m );
				}
				...
				for ( int i = 0; i < targets.Count; ++i )
				{
					...
					else
					{
						...
						if ( m.Hits < m.HitsMax )
						{
							...
							Utility.FixMinMax( ref toHeal, 8, 24 );
