...
namespace Server.Spells.Bushido
{
	public class Evasion : SamuraiSpell
	{
		...
		{
			if ( !VerifyCooldown( Caster ) )
				return false;

			if ( !VerifyCast( Caster ) )
				return false;

			return base.CheckCast();
		}

		private static bool VerifyCooldown( Mobile from, bool messages = true )
		{
			if ( m_Table3[from] != null )
			{
				DateTime date = (DateTime) m_Table3[from];
		...
		private static bool VerifyCast( Mobile from, bool messages = true )
		{
			if ( from == null ) // sanity
				return false;
			...
			BaseWeapon weap = from.FindItemOnLayer( Layer.OneHanded ) as BaseWeapon;

			if ( weap == null )
				weap = from.FindItemOnLayer( Layer.TwoHanded ) as BaseWeapon;

			if ( weap != null )
			{
				if ( from.Skills[weap.Skill].Base < 50.0 )
				{
					// UBW and Mage weapon have no bearing on this, nor do skill items and such.
					if ( messages )
						from.SendLocalizedMessage( 1076206 ); // Your skill with your equipped weapon must be 50 or higher to use Evasion.
			...
			if ( messages )
				from.SendLocalizedMessage( 1062944 ); // You must have a weapon or a shield equipped to use this ability!
		...
		private static Hashtable m_Table3 = new Hashtable();

		public static bool IsRecovering( Mobile m )
		{
