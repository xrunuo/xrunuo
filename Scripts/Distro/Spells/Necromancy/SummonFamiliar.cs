...
namespace Server.Spells.Necromancy
{
	public class SummonFamiliarSpell : NecromancerSpell
	{
		...
		public static Hashtable m_TableTimer = new Hashtable();

		private void Expire_Callback( object state )
		{
			Mobile m = (Mobile) state;

			m.CloseGump<SummonFamiliarGump>();

			m_TableTimer.Remove( m );
		}
		...
		public override void OnCast()
		{
			if ( CheckSequence() )
			{
				...
				Timer timer = (Timer) m_TableTimer[Caster];

				if ( timer != null )
					timer.Stop();

				m_TableTimer[Caster] = timer = Timer.DelayCall( TimeSpan.FromSeconds( 60.0 ), new TimerStateCallback( Expire_Callback ), Caster );
			}
	...
	{
		public override int TypeID { get { return 0x292; } }
		...
		public SummonFamiliarGump( Mobile from, SummonFamiliarEntry[] entries )
			: base( 200, 100 )
		{
			m_From = from;
			m_Entries = entries;
