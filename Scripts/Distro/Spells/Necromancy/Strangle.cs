		...
		private class InternalTimer : Timer
		{
				...
				: base( TimeSpan.FromSeconds( 0.1 ), TimeSpan.FromSeconds( 0.1 ) )
			{
				...
				for ( int i = m_Count; i > 0; i-- )
					buffdelay += (int) ( Math.Ceiling( ( 1.0 + ( 5 * i ) ) / m_MaxCount ) );

				BuffInfo.AddBuff( target, new BuffInfo( BuffIcon.Strangle, 1075794, TimeSpan.FromSeconds( buffdelay ), target, String.Format( "{0}\t{1}", m_MinBaseDamage, m_MaxBaseDamage ) ) );
			}
