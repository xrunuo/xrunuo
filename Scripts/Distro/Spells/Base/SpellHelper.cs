...
namespace Server.Spells
{
	...
	{
		private static TimeSpan DamageDelay = TimeSpan.FromSeconds( 1.0 );
		...
		public static bool CheckCombat( Mobile m )
		{
			...
			for ( int i = 0; i < m.Aggressors.Count; ++i )
			{
				...
				if ( info.Attacker.IsPlayer && ( DateTime.Now - info.LastCombatTime ) < CombatHeatDelay )
					return true;
			}
		...
		{
			int offset = bonus;
			string name = String.Format( "[Magic] {0} Bonus", type );
		...
		{
			int offset = -curse;
			string name = String.Format( "[Magic] {0} Malus", type );
		...
		private static TravelValidator[] m_Validators = new TravelValidator[]
			{
				...
				new TravelValidator( IsDoomDarkGuardianRoom ),
...
/* Tele To */		{ true, 	true, 		true, 		true, 		true, 			true, 			true, 		false, 				true, 				false,				false, 			true,			false, 			true, 				false, 			false,			true,			false,			false,			false }
		...
		public static bool IsMLDungeon( Map map, Point3D loc )
		{
			...
			// Felucca and Trammel
			if ( ( map == Map.Felucca || map == Map.Trammel ) && x > 6144 )
				return true;

			// Twisted Weald (Ilshenar)
			if ( map == Map.Ilshenar && x > 2110 && y > 1150 && x < 2247 && y < 1292 )
				return true;

			// Labyrinth (Malas)
			if ( map == Map.Malas && x > 256 && y > 1792 && x < 510 && y < 2046 )
				return true;

			// Bedlam (Malas)
			if ( map == Map.Malas && x > 77 && y > 1594 && x < 212 && y < 1778 )
				return true;

			// The Cidatel (Tokuno)
			if ( map == Map.Malas && x > 0 && y > 1792 && x < 256 && y < 2048 )
				return true;
