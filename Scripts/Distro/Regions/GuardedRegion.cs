	...
	public class GuardedRegion : BaseRegion
	{
		...
		public override void MakeGuard( Mobile focus )
		{
			...
			if ( useGuard == null )
			{
				...
				try { Activator.CreateInstance( m_GuardType, m_GuardParams ); }
				catch { }
			}
			else
			...
			if ( IsGuardCandidate( m ) )
			{
				...
				if ( timer == null )
				{
					...
					if ( map != null )
					{
						...
						{
							if ( !v.IsPlayer && v != m && !IsGuardCandidate( v ) && ( ( v is BaseCreature ) ? ( (BaseCreature) v ).IsHumanInTown() : ( v.Body.IsHuman && v.Region.IsPartOf( this ) ) ) )
							{
