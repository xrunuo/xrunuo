...
namespace Server.Mobiles
{
	...
	public class GlobalTownCrierEntryList : ITownCrierEntryList
	{
		...
		public List<TownCrierEntry> Entries { get { return m_Entries; } }
	...
	public class TownCrier : Mobile, ITownCrierEntryList
	{
		...
		public TownCrierEntry AddEntry( string[] lines, TimeSpan duration )
		{
			return AddEntry( new TownCrierEntry( lines, duration ) );
		}

		public TownCrierEntry AddEntry( TownCrierEntry tce )
		{
			if ( m_Entries == null )
			{
				m_Entries = new List<TownCrierEntry>();
		...
		{
			return ( m_NewsTimer == null && from.Alive && this.InRange( from, 12 ) );
		}

		public override void OnSpeech( SpeechEventArgs e )
		{
			if ( m_NewsTimer == null && e.HasKeyword( 0x30 ) && e.Mobile.Alive && this.InRange( e.Mobile, 12 ) ) // *news*
			{
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			if ( m_Entries != null )
			{
				...
				foreach ( TownCrierEntry entry in m_Entries )
					entry.Serialize( writer );
			}
			else
				writer.Write( (int) 0 ); // m_Entries.Count
		}
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			if ( version == 1 )
			{
				...
				for ( int i = 0; i < entryCount; i++ )
					AddEntry( new TownCrierEntry( reader ) );
			}
		}
	}
}
