...
namespace Server.Mobiles
{
	public class Spawner : Item
	{
		...
		[Constructable]
		public Spawner( int amount, int minDelay, int maxDelay, int team, int homeRange, string creatureName )
			: base( 0x1f13 )
		{
		...
		[Constructable]
		public Spawner( string creatureName )
			: base( 0x1f13 )
		{
			ArrayList creaturesName = new ArrayList();
			creaturesName.Add( creatureName.ToLower() );
		...
		public Spawner( int amount, TimeSpan minDelay, TimeSpan maxDelay, int team, int homeRange, ArrayList creaturesName )
			: base( 0x1f13 )
		{
		...
		private class WarnTimer : Timer
		{
			...
			protected override void OnTick()
			{
				try
				{
					...
					using ( StreamWriter op = new StreamWriter( Path.Combine( Core.Config.LogDirectory, "badspawn.log" ), true ) )
					{
