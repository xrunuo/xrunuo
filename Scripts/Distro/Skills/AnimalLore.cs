...
namespace Server.SkillHandlers
{
	...
	{
		public override int TypeID { get { return 0x1DB; } }
		...
		private static string FormatAttributes( int cur, int max )
		{
			...
			return String.Format( "<div align=right>{0}/{1}</div>", FormatValue( cur ), FormatValue( max ) );
		}
		...
		private static string FormatDouble( double val )
		{
			...
			return String.Format( "<div align=right>{0:F1}</div>", val ).Replace( ",", "." );
		}
			...
			: base( 250, 50 )
		{
			...
			AddHtmlLocalized( 153, y, 160, 18, ( !c.Controlled || c.Loyalty == PetLoyalty.None ) ? 1061643 : 1049594 + (int) c.Loyalty, LabelColor, false, false );
