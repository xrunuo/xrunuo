		...
		private class InternalTargetPoison : Target
		{
			...
			private class InternalTarget : Target
			{
				...
				protected override void OnTarget( Mobile from, object targeted )
				{
					...
					if ( targeted is Food || targeted is FukiyaDart || targeted is Shuriken )
					{
						startTimer = true;
					}
					...
					protected override void OnTick()
					{
						if ( m_From.CheckTargetSkill( SkillName.Poisoning, m_Target, m_MinSkill, m_MaxSkill ) )
						{
							...
							else if ( m_Target is FukiyaDart )
							{
								...
								( (FukiyaDart) m_Target ).PoisonCharges = Math.Min( 18 - ( m_Poison.Level * 2 ), ( (FukiyaDart) m_Target ).UsesRemaining );
							}
