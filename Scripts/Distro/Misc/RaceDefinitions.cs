	...
	public class RaceDefinitions
	{
		...
		private class Gargoyle : Race
		{
			public Gargoyle( int raceID, int raceIndex )
				: base( raceID, raceIndex, "Gargoyle", "Gargoyles", 666, 667, 694, 695/*, Expansion.SA*/ )
			{
			}
			...
			public override bool ValidateHair( bool female, int itemID )
			{
				...
				if ( !female && itemID >= 0x4258 && itemID <= 0x425F )
					return true;

				if ( female )
					for ( int i = 0; i < m_FemaleHorns.Length; i++ )
						if ( m_FemaleHorns[i] == itemID )
							return true;
			...
			{
				if ( !female )
					return Utility.Random( 0x4258, 8 );
				else
					return Utility.RandomList( m_FemaleHorns );
			}

			private static int[] m_FemaleHorns = new int[]
				{
					0x4261,	0x4262,	0x4273, 0x4274,
					0x4275, 0x42AA, 0x42AB, 0x42B1
				};
			...
			public override bool ValidateFacialHair( bool female, int itemID )
			{
				...
				return itemID >= 0x42AD && itemID <= 0x42B1;
			}
			...
			public override int RandomFacialHair( bool female )
			{
				...
				return Utility.Random( 0x42AD, 5 );
			}
			...
			public override int RandomSkinHue()
			{
				return Utility.Random( 0x6DB, 24 );
			}
