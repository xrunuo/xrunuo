...
namespace Server.Gumps
{
	public abstract class BaseVendorRentalGump : Gump
	{
		...
		protected BaseVendorRentalGump( GumpType type, VendorRentalDuration duration, int price, int renewalPrice, Mobile landlord, Mobile renter, bool landlordRenew, bool renterRenew, bool renew )
			: base( 100, 100 )
		{
			/*0*/
			Intern( "" );
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			if ( info.ButtonID >= 1 && info.ButtonID <= 4 ) // Contract duration
			{
			...
			else
			{
				switch ( info.ButtonID )
				{
					...
					case 6: // Price Per Rental
						SetPricePerRental( from );
						break;

					case 8: // Renew on expiration - renter
						RenterRenewOnExpiration( from );
						break;

					case 9: // Renew on expiration - landlord
						LandlordRenewOnExpiration( from );
						break;

					case 10: // Offer Contract To Someone
						OfferContract( from );
						break;

					case 11: // Accept offer
						AcceptOffer( from );
						break;

					case 12: // Cancel offer
						Cancel( from );
						break;

					case 13: // Renewal price
	...
	public class VendorRentalContractGump : BaseVendorRentalGump
	{
		...
		{
			from.Prompt = new PricePerRentalPrompt( from, m_Contract );
		}
	...
	public class VendorRentalOfferGump : BaseVendorRentalGump
	{
		...
		protected override void AcceptOffer( Mobile from )
		{
			...
			var house = HousingHelper.FindHouseAt( m_Contract );
			if ( house == null )
			{
				return;
			}
	...
	public class LandlordVendorRentalGump : BaseVendorRentalGump
	{
		...
		private class ContractRenewalPricePrompt : Prompt
		{
			...
			public override void OnResponse( Mobile from, string text )
			{
				...
				try { price = Convert.ToInt32( text ); }
				catch { price = -1; }
	...
	{
		public override int TypeID { get { return 0x2AC; } }
