...
namespace Server.Gumps
{
	...
	public class WarningGump : Gump
	{
		...
		public WarningGump( object header, int headerColor, object content, int contentColor, int width, int height, WarningGumpCallback callback, object state )
			: this( header, headerColor, content, contentColor, width, height, callback, state, true )
		{
		}

		public WarningGump( object header, int headerColor, object content, int contentColor, int width, int height, WarningGumpCallback callback, object state, bool cancelButton )
			: base( ( 640 - width ) / 2, ( 480 - height ) / 2 )
		{
			...
			if ( header is int )
				AddHtmlLocalized( 10, 10, width - 20, 20, (int) header, headerColor, false, false );
			else if ( header is string )
				AddHtml( 10, 10, width - 20, 20, String.Format( "<BASEFONT COLOR=#{0:X6}>{1}</BASEFONT>", 0xAA2222, header ), false, false );
