...
namespace Server.Gumps
{
	...
	public class AdminGump : Gump
	{
		...
		private static string[] m_AccessLevelStrings = new string[] { "Player", "Counselor", "Game Master", "Seer", "Administrator", "Developer", "Owner" };
			...
			: base( 50, 40 )
		{
			...
			switch ( pageType )
			{
				...
				case AdminGumpPage.Information_Perf:
					{
						AddLabel( 20, 130, LabelHue, "Incoming Traffic: " + Utility.FormatByteAmount( GameServer.Instance.Incoming ) );
						AddLabel( 20, 150, LabelHue, "Outgoing Traffic: " + Utility.FormatByteAmount( GameServer.Instance.Outgoing ) );
						...
						lock ( pools )
						{
							for ( int i = 0; i < pools.Count; ++i )
							{
								...
								sb.Append( name );
								sb.Append( "<br>Size: " );
								sb.Append( Utility.FormatByteAmount( bufferSize ) );
								...
								sb.Append( "% : " );
								sb.Append( Utility.FormatByteAmount( ( currentCapacity - freeCount ) * bufferSize ) );
								sb.Append( " of " );
								sb.Append( Utility.FormatByteAmount( currentCapacity * bufferSize ) );
							}
						}

						AddLabel( 20, 180, LabelHue, "Buffer Pools:" );
						AddHtml( 20, 200, 380, 150, sb.ToString(), true, true );
				...
				case AdminGumpPage.Clients:
					{
						if ( m_List == null )
						{
							m_List = new ArrayList( GameServer.Instance.Clients.ToArray() );
				...
				case AdminGumpPage.Accounts:
					{
						if ( m_List == null )
						{
							m_List = new ArrayList( Accounts.GetAccounts().ToArray() );
				...
				case AdminGumpPage.AccountDetails_ChangePassword:
					{
						...
						if ( a.GetTag( "email" ) != null )
							AddButtonLabeled( 20, 350, GetButtonID( 5, 33 ), "Assign Automatic Password" );
									...
									else
									{
										foreach ( GameClient ns in GameServer.Instance.Clients )
										{
											...
											if ( forName )
											{
												...
												isMatch = ( m != null && m.Name.ToLower().IndexOf( match ) >= 0 ) || ( a != null && a.Username.ToLower().IndexOf( match ) >= 0 );
											}
											else
											{
				...
				case 5:
					{
						switch ( index )
						{
							...
							case 6:
								{
									...
									else
									{
										...
										{
											dispAccount = Accounts.CreateAccount( un, pw );
							...
							case 33: // asign automatic password
								{
									Account a = m_State as Account;
									...
									a.SetPassword( password );
									SendEmail( a, password );

									CommandLogging.WriteLine( from, "{0} {1} sending automatic new pass for account {2}", from.AccessLevel, CommandLogging.Format( from ), a.Username );

									from.SendGump( new AdminGump( from, AdminGumpPage.AccountDetails_Information, 0, null, "The password has been changed.", m_State ) );
							...
							case 31: // View all inactive accounts
								{
									...
									DateTime minTime = DateTime.Now - ( Misc.TestCenter.Enabled ? TimeSpan.FromMinutes( 15.0 ) : Environment.Config.AccountDecay );

									foreach ( Account acct in Accounts.GetAccounts() )
									{
										if ( acct.LastLogin <= minTime && !acct.Banned && !acct.FeatureEnabled( AccountFeature.NoExpire ) ) // Ignore banned accounts
											results.Add( acct );
									}
		...
		private static readonly char[] m_AllChars = "abcdefghijklmnopqrstuvwxyz0123456789".ToCharArray();
		...
		private static string GetRandomPassword()
		{
			...
			for ( int i = 0; i < 10; i++ )
				password = String.Format( "{0}{1}", password, m_AllChars[Utility.RandomMinMax( 0, m_AllChars.Length - 1 )] );
